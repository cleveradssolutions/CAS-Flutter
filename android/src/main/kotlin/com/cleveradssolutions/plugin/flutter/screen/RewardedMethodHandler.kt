...
class RewardedMethodHandler(
    ...
    override fun initInstance(id: String): Ad<CASRewarded> {
        ...
        rewarded.contentCallback = callback
        rewarded.onImpressionListener = callback
    ...
    private fun load(rewarded: CASRewarded, result: MethodChannel.Result) {
        rewarded.load(contextService.application)

        result.success()
    }

    private fun show(ad: Ad<CASRewarded>, result: MethodChannel.Result) {
        val onRewardEarnedListener = ad.ad.contentCallback as OnRewardEarnedListener
        ad.ad.show(contextService.activity, onRewardEarnedListener)
