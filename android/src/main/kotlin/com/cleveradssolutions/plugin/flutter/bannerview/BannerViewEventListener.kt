...
class BannerViewEventListener: AdViewListener, EventChannel.StreamHandler {
    ...
    override fun onAdViewFailed(view: CASBannerView, error: AdError) {
        eventSink?.success(mapOf("id" to flutterIds[view.id], "event" to "onAdViewFailed", "data" to error.message))
    }

    override fun onAdViewPresented(view: CASBannerView, info: AdImpression) {
        eventSink?.success(mapOf("id" to flutterIds[view.id], "event" to "onAdViewPresented", "data" to info))
    }
