...
extension FlutterMethodCall {
    ...
    func getArgAndCheckNil<T>(_ name: String, _ result: FlutterResult) -> T? {
        if let args = arguments as? [String: Any], let arg = args[name] as? T {
            return arg
        } else {
    ...
    @inline(__always)
    func getArgAndReturn<T>(_ name: String, _ result: FlutterResult, _ action: (T) -> Void) {
        getArgAndReturnResult(name, result) { value in action(value); return nil }
    }

    /// Try get argument from call, do action with it and return null.
    @inline(__always)
    func getArgAndReturn<T, T2>(_ name1: String, _ name2: String, _ result: FlutterResult, _ action: (T, T2) -> Void) {
